...
## 리펙토링
- 테스트코드 : 모든 함수와 기능과 추가될 함수와 기능에 대해 테스트 코드를 작성
- 보안, 로깅 : 보안 대처와 로깅 기능의 적용이 필요 - Spring security를 활용해 로그인을 구현, 503, 404 등의 화면 이동 제어 필요
- 게시판 분리 : 사용자가 볼 게시글 화면과 관리자로써 확인할 게시글 분리
- 도메인 주도 : 도메인 - DTO, DAO, VO 등의 개념 적용을 확인, 이를 바탕으로 개발
- 추가 기능 구현 : 댓글 기능 구현, 기존의 타입을 변경 - 이미지 포함 가능한 컨텐츠로 변경할 예정